...
function minify(inputDir, outputDir, tempDir, verbose, shrink, name){
    ...
    fs.readdir(inputDir, function(err, files){
        if(err){
                ...
                if(minify){
                    ...
                    if(body && body.length > 0){
                        ...
                        if(body && body.length > 1){
                            ...
                            data = data.replace(test, "<script async src=\"" + name + ".min.js?v" + version + "\"></script>");
                        }
                    }
                }
            ...
            if(shrink){
                shrunk += "\npageLoad();";
                fs.writeFileSync(path.join(outputDir, name + ".min.js"), shrunk, {encoding:"utf8"});
                fs.writeFileSync(path.join(tempDir, name + ".min.js"), shrunk, {encoding:"utf8"});
            }
